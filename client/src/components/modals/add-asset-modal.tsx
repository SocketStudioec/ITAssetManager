...
interface AddAssetModalProps {
  ...
  companyId: string;
  defaultType?: "physical" | "application" | "license" | "contract";
}

export default function AddAssetModal({ open, onOpenChange, companyId, defaultType }: AddAssetModalProps) {
  ...
  const getDefaultValues = () => ({
    companyId: companyId || "",
    name: "",
    type: (defaultType || "physical") as "physical" | "application" | "license" | "contract",
    description: "",
    serialNumber: "",
    model: "",
    manufacturer: "",
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {defaultType === "application" ? "Agregar Nueva Aplicación" : "Agregar Nuevo Activo"}
          </DialogTitle>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit, (errors) => {
            ...
            {/* Selector de tipo solo si no viene defaultType */}
            {!defaultType && (
              <FormField
            ...
            {/* Campos específicos para aplicaciones */}
              ...
              <div className="space-y-4 p-4 bg-muted/30 rounded-lg">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="https://..." {...field} data-testid="input-application-url" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="v1.0.0" {...field} data-testid="input-application-version" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-4 text-sm">
                    ...
                    <FormField control={form.control} name="domainCost" render={({ field }) => (
                      <FormItem><FormControl>
                        <Input type="number" step="0.01" placeholder="0.00" {...field}
                          onChange={(e) => field.onChange(e.target.value === "" ? undefined : parseFloat(e.target.value))}
                          data-testid="input-domain-cost" className="h-8" />
                      </FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={form.control} name="domainExpiry" render={({ field }) => (
                      <FormItem><FormControl>
                        <Input type="date" {...field}
                          value={field.value ? new Date(field.value).toISOString().split('T')[0] : ''}
                          onChange={(e) => field.onChange(e.target.value ? new Date(e.target.value) : undefined)}
                          data-testid="input-domain-expiry" className="h-8" />
                      </FormControl><FormMessage /></FormItem>
                    )} />

                    {/* SSL */}
                    <div className="flex items-center">SSL</div>
                    <FormField control={form.control} name="sslCost" render={({ field }) => (
                      <FormItem><FormControl>
                        <Input type="number" step="0.01" placeholder="0.00" {...field}
                          onChange={(e) => field.onChange(e.target.value === "" ? undefined : parseFloat(e.target.value))}
                          data-testid="input-ssl-cost" className="h-8" />
                      </FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={form.control} name="sslExpiry" render={({ field }) => (
                      <FormItem><FormControl>
                        <Input type="date" {...field}
                          value={field.value ? new Date(field.value).toISOString().split('T')[0] : ''}
                          onChange={(e) => field.onChange(e.target.value ? new Date(e.target.value) : undefined)}
                          data-testid="input-ssl-expiry" className="h-8" />
                      </FormControl><FormMessage /></FormItem>
                    )} />

                    {/* Hosting */}
                    <div className="flex items-center">Hosting</div>
                    <FormField control={form.control} name="hostingCost" render={({ field }) => (
                      <FormItem><FormControl>
                        <Input type="number" step="0.01" placeholder="0.00" {...field}
                          onChange={(e) => field.onChange(e.target.value === "" ? undefined : parseFloat(e.target.value))}
                          data-testid="input-hosting-cost" className="h-8" />
                      </FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={form.control} name="hostingExpiry" render={({ field }) => (
                      <FormItem><FormControl>
                        <Input type="date" {...field}
                          value={field.value ? new Date(field.value).toISOString().split('T')[0] : ''}
                          onChange={(e) => field.onChange(e.target.value ? new Date(e.target.value) : undefined)}
                          data-testid="input-hosting-expiry" className="h-8" />
                      </FormControl><FormMessage /></FormItem>
                    )} />

                    {/* Servidores */}
                    <div className="flex items-center">Servidores</div>
                    <FormField control={form.control} name="serverCost" render={({ field }) => (
                      <FormItem><FormControl>
                        <Input type="number" step="0.01" placeholder="0.00" {...field}
                          onChange={(e) => field.onChange(e.target.value === "" ? undefined : parseFloat(e.target.value))}
                          data-testid="input-server-cost" className="h-8" />
                      </FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={form.control} name="serverExpiry" render={({ field }) => (
                      <FormItem><FormControl>
                        <Input type="date" {...field}
                          value={field.value ? new Date(field.value).toISOString().split('T')[0] : ''}
                          onChange={(e) => field.onChange(e.target.value ? new Date(e.target.value) : undefined)}
                          data-testid="input-server-expiry" className="h-8" />
                      </FormControl><FormMessage /></FormItem>
                    )} />
                  </div>
                </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      {selectedType === "application" ? "Costo Contable (App)" : "Costo Contable"}
                    </FormLabel>
                    <FormControl>
                      <Input
                        type="number"
