...
import ViewAssetModal from "@/components/modals/view-asset-modal";
...
import { Plus, Search, Filter, Edit2, Trash2, Eye, ExternalLink, AlertTriangle, Calendar, Laptop, DollarSign, CheckCircle } from "lucide-react";
...
export default function Applications() {
  ...
  // Cálculos de costos para aplicaciones
  const monthlyRealApplications = applications.reduce((sum: number, a: any) => 
    sum + 
    Number(a.monthly_cost || 0) + 
    Number(a.domain_cost || 0) + 
    Number(a.ssl_cost || 0) + 
    Number(a.hosting_cost || 0) + 
    Number(a.server_cost || 0), 0);
  const annualRealApplications = monthlyRealApplications * 12;
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto bg-background">
          <div className="p-6">
            ...
            {/* ====== CARDS DE RESUMEN DE APLICACIONES ====== */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">

              {/* Card 1: Total de Aplicaciones */}
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-foreground">
                        {isAssetsLoading ? "..." : applications.length}
                      </p>
                    </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-foreground">
                        {isAssetsLoading ? "..." : applications.filter((a: any) => a.status === "active").length}
                      </p>
                    </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-foreground">
                        ${isAssetsLoading ? "..." : monthlyRealApplications.toLocaleString()}
                      </p>
                    </div>
                    <div className="w-12 h-12 bg-chart-2 rounded-lg flex items-center justify-center flex-shrink-0">
              ...
              {/* Card 4: Costo Anual Real */}
              <Card className="border-border">
                <CardContent className="p-6">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-sm font-medium text-muted-foreground">Costo Anual Real</p>
                      <p className="text-2xl font-bold text-foreground">
                        ${isAssetsLoading ? "..." : annualRealApplications.toLocaleString()}
                      </p>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {isAssetsLoading ? (
                ...
                filteredApplications.map((app: any) => (
                  <Card key={app.id} className={`border-border hover:shadow-md transition-shadow ${getExpiryWarning(app)?.type === 'expired' ? 'border-destructive' :
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-2 mb-1">
                            ...
                            {getExpiryWarning(app) && (
                              <AlertTriangle className={`w-4 h-4 ${getExpiryWarning(app)?.type === 'expired' ? 'text-destructive' : 'text-chart-4'
                                }`} />
                            )}
                          </div>
                                ...
                                {Number(app.domain_cost) > 0 && (
                                  <div className="flex justify-between items-center">
                                    ...
                                    <div className="text-right">
                                      ...
                                      {app.domain_expiry && (
                                        <div className={`text-xs ${new Date(app.domain_expiry) < new Date() ? 'text-destructive' :
                                ...
                                {Number(app.ssl_cost) > 0 && (
                                  <div className="flex justify-between items-center">
                                    ...
                                    <div className="text-right">
                                      ...
                                      {app.ssl_expiry && (
                                        <div className={`text-xs ${new Date(app.ssl_expiry) < new Date() ? 'text-destructive' :
                                ...
                                {Number(app.hosting_cost) > 0 && (
                                  <div className="flex justify-between items-center">
                                    ...
                                    <div className="text-right">
                                      ...
                                      {app.hosting_expiry && (
                                        <div className={`text-xs ${new Date(app.hosting_expiry) < new Date() ? 'text-destructive' :
                                ...
                                {Number(app.server_cost) > 0 && (
                                  <div className="flex justify-between items-center">
                                    ...
                                    <div className="text-right">
                                      ...
                                      {app.server_expiry && (
                                        <div className={`text-xs ${new Date(app.server_expiry) < new Date() ? 'text-destructive' :
                      ...
                      <div className="flex items-center justify-between mt-4 pt-4 border-t border-border">
                        <div className="flex space-x-2">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => setSelectedAssetForView(app)}
      ...
      {/* Modal para ver aplicación */}
      {selectedAssetForView && (
        <ViewAssetModal
          open={!!selectedAssetForView}
          onOpenChange={(open) => !open && setSelectedAssetForView(null)}
